...
4. **İkon 20×20 + component-özel `.bt-dd-option__icon`** → global standart `.bt-icon` (24×24 wrapper, SVG 16×16) içinde yeni `.bt-dropdown-list-item__control` (32×32 slot) — ham SVG asset'i indirilip gerçek boyut ölçüldü (~15×15, 16'ya yeterince yakın, ayrı bir override gerekmedi — Alert'teki 18×18 sapmasının aksine).
...
**Kapsam kararı:** `_ddOptionsHtml` (Dialog form örneği ve Data Table InCell Editing'in status dropdown'ı tarafından da reuse edilen paylaşılan sabit) yalnızca YENİ class yapısına taşındı (`.bt-dropdown-list-item__control` > `.bt-icon`, `--has-left`) — mevcut 4 seçenek/ilk-seçili içeriği DEĞİŞMEDİ, yeni bir Hover/Focus/Disabled demo'su İCAT EDİLMEDİ (paylaşılan sabit olduğu için Dialog/Data Table'ın görünümünü beklenmedik şekilde değiştirmemek adına). Data Table'ın kendi "Basic" (ikonsuz) status dropdown'ı (`_gridEditDropdownHtml`, `.bt-dropdown-list-item` — `--has-left` YOK) yeni CSS'le otomatik doğru render oluyor (8px padding varsayılanı zaten "Basic" content type'a karşılık geliyor). `docs/css/styles.css` ("DROPDOWN LIST PANEL" bloğu), `docs/js/pages-web.js` (`_ddIconLoader`, `_ddOptionsHtml`, Dropdown sayfası CSS Properties tab'ına yeni "Options Panel & List Item" tablosu) güncellendi. **Doğrulama:** `node --check` temiz; tarayıcıda `getComputedStyle` ile panel padding (4px), border (`#f5f5f5`), box-shadow (shadow-md), margin-top (4px), seçili item arka planı (`#e6e6e6`), ikon boyutu (16×16) tek tek ölçüldü — hepsi doğru; Dialog örneği ve Data Table InCell Editing'in status dropdown'ı canlı açılıp regresyon olmadığı (Basic ve Left-Control varyantlarının ikisi de) doğrulandı; konsolda JS hatası yok.

### 22.8 İsimlendirme düzeltmesi — kısaltmalar kaldırıldı, paylaşılan wrapper'lara kimlik class'ı eklendi (2026-09-17, kullanıcı geri bildirimi)

Kullanıcı §22.7'deki `bt-dd-*` kısaltmasına itiraz etti ("dropdown list bunun adı... sürekli kafana göre isim değişiklikleri kısaltmalar yapıyorsun") — haklı: `bt-dd-option`/`bt-dd-options` Figma'nın kendi katman adlarını ("Dropdown List", "Dropdown List Item") yansıtmıyordu. Düzeltme (yalnızca isimlendirme, davranış/görsel DEĞİŞMEDİ):

- `.bt-dd-options` → `.bt-dropdown-list`
- `.bt-dd-option` → `.bt-dropdown-list-item` (+ `__control`, `__text`, tüm `--` modifier'ları aynı desende yeniden adlandırıldı)
